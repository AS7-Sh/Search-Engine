...
def vectorize_query(query: str):
    tfidf_model = None
    with open('D:/antique/tfidf/tfidf_model.pickle', 'rb') as handle:
        tfidf_model = pickle.load(handle)
    return tfidf_model.transform([query]).toarray()[0];


with open('D:/antique/tfidf/tfidf_matrix.pickle', 'rb') as handle:
...
qrels = {}
with open('D:/antique/test/qrels', 'r') as f:
    for line in f:
...
def get_query(query_id):
    with open('D:/antique/test/queries.txt', newline='') as csvfile:
